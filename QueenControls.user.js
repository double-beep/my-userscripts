// ==UserScript==
// @name        Queen Controls
// @author      double-beep (modified)
// @namespace   http://tinygiant.io
// @description Adds quick links for [tp|fp|nc] to @Queen reports.
// @include     http*://chat.stackoverflow.com/rooms/111347/*
// @version     1.2
// @grant       none
// ==/UserScript==
/* jshint esnext: true */
        ...
        fire: function(...args) {
            ...
            function eventHandler(event) {
                if(event.room_id !== CHAT.CURRENT_ROOM_ID || // event is not in this room
                   ...
                   event.user_id !== 6294609) return;        // event is not from Queen

                const content = document.createElement('div');
                content.innerHTML = event.content;

                if(!/^\[ Heat Detector/.test(content.textContent.trim())) return; // event is not a report
                ...
                setTimeout(() => {
                    ...
                    wrap.appendChild(document.createTextNode(' | '));
                    wrap.appendChild(createLink('nc'));
                    wrap.appendChild(document.createTextNode(' | '));
                    wrap.appendChild(createLink('sk'));
                    wrap.appendChild(document.createTextNode(' ] '));
...
script.textContent = `(${ QueenControls.toString() })();`;
